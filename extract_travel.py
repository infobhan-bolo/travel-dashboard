...
def build_data():
    ...
    for start_d, end_d in months:
        ...
        cur = start_d
        while cur < end_d:
            ishir = any(a <= cur <= b for a, b in info['ishir']['ranges'])
            duyen = any(a <= cur <= b for a, b in info['duyen']['ranges'])
            if ishir and duyen:
                days[str(cur.day)] = 'both'
            elif ishir:
                days[str(cur.day)] = 'ishir'
            elif duyen:
                days[str(cur.day)] = 'duyen'

            items = []
            for key, label in [('ishir', 'Ishir'), ('duyen', 'Duyen')]:
                seen = set()
